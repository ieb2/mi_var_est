...
Please see the following [link](https://mi-var-est.shinyapps.io/mi_var_est/) for an interactive presentation of the results accompanied by visuals, and [Data] for the complete set of results only. 

## General observations 
On a general note, somewhat poor confidence interval coverage was noted across all methods and conditions, particularly compared to other results in the literature; however, this is likely caused by the rather low number of iterations the Monte Carlo simulation was allowed to proceed through, as well as the sub-optimal conditions the methods were allowed to go through. For instance, in one comparable study, the authors performed 10,000 iterations of the Monte Carlo study, while this study was only able to perform 333 due to memory issues. Likewise, the foregoing study used 200 resamples for bootstrap-based methods, while this study was only able to use 5 bootstrap resamples due to challenges in memory. Finally, although somewhat less drastic compared to the other differences, this study was only able to use 5 multiply imputed datasets, while the foregoing study in the literature was able to use 10. 

Given the foregoing conditions, however, the results obtained here were still consistent with what was noted in the literature.  The lower coverage obtained, as well as the small biases noted in the point estimators are likely explained by the high Monte Carlo standard error caused due to the small number of iterations. 

Regardless, we noted the following trends in the simulation as being worthwhile for further study. An increased sample size led to narrower confidence intervals, which is reasonable given the inverse square root of the sample size relationship between confidence interval width and sample size. CI depends on (among other things) the standard error of an appropriate estimator. As you know, the standard error has an inverse relationship with the sample size and so will decrease as n gets larger. At an intuitive level, a larger sample will contain more information about the population parameter of interest, and we can therefore be more precise in our estimation of it. Somewhat paradoxically, however, increased sample size led to higher coverage probability, although the confidence intervals got narrower. This is likely due to improvements in the point estimation. 

It was also noted that at higher percentages of missing data, Rubin's rules began producing less accurate point estimates with narrower confidence intervals compared to bootstrap-based methods. 

In general, across most conditions, it was noted that the bootstrap MI pooled method had the highest coverage probability under uncongeniality; however, also had the least accurate point estimates which improved as the sample size increased.  
...
## Formal Definition of Congeniality {#formal} 
